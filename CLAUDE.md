...
1. **`poll-jira.sh <project-dir>`** — calls Jira REST API, pushes new tasks to `.claude-queue.jsonl`
2. **`poll-github.sh <project-dir>`** — calls GitHub API, pushes new PR events to `.claude-queue.jsonl`
...
| `scripts/queue.sh` | FIFO queue backed by `.claude-queue.jsonl` with `flock` locking. Operations: `push`, `pop`, `count`, `peek` |
| `scripts/session-setup.sh` | Sets `GH_TOKEN` and `ANTHROPIC_API_KEY` from `~/.config/` |
...
| `~/.config/atlassian-api-token-admin` | `poll-jira.sh` (Jira REST API basic auth) |
